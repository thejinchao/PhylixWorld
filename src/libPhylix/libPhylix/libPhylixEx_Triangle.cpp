...
b2Triangle::b2Triangle(Real x1, Real y1, Real x2, Real y2, Real x3, Real y3){
	Real dx1 = x2-x1;
	Real dx2 = x3-x1;
	Real dy1 = y2-y1;
	Real dy2 = y3-y1;
	Real cross = dx1*dy2-dx2*dy1;
	bool ccw = (cross>0);
	if (ccw){
...
bool b2Triangle::IsInside(Real _x, Real _y){
	if (_x < x[0] && _x < x[1] && _x < x[2]) return false;
	if (_x > x[0] && _x > x[1] && _x > x[2]) return false;
	if (_y < y[0] && _y < y[1] && _y < y[2]) return false;
	if (_y > y[0] && _y > y[1] && _y > y[2]) return false;
		
		Real vx2 = _x-x[0]; Real vy2 = _y-y[0];
		Real vx1 = x[1]-x[0]; Real vy1 = y[1]-y[0];
		Real vx0 = x[2]-x[0]; Real vy0 = y[2]-y[0];
		
		Real dot00 = vx0*vx0+vy0*vy0;
		Real dot01 = vx0*vx1+vy0*vy1;
		Real dot02 = vx0*vx2+vy0*vy2;
		Real dot11 = vx1*vx1+vy1*vy1;
		Real dot12 = vx1*vx2+vy1*vy2;
